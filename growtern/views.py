...
def submit_task(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('task_submission')  # avoid resubmitting on refresh
    else:
        form = TaskSubmissionForm()

    # Get all submissions
    submitted_tasks = TaskSubmission.objects.select_related('task', 'intern').all().order_by('-id')
